...
def get_route_from_mapmyindia(start_coords, end_coords):
    """
    calling to MapMyIndia routing API.
    Returns the map visualization coordinates on mappls connectivity visualization interface and if api connected succesfually so connect then & return the response as if it came from the server.
    """
